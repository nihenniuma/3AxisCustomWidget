...
vtkBoxRepresentation::vtkBoxRepresentation()
{
  ...
  this->AxisLine = new vtkActor* [3];
  this->AxisLineSource = new vtkLineSource* [3];
  this->AxisLineMapper = new vtkPolyDataMapper* [3];

  for(int i = 0; i < 3; ++i){
    // Circle Axis
    this->AxisPolygonSource[i] = vtkRegularPolygonSource::New();
    this->AxisPolygonSource[i]->SetNumberOfSides(100);
    ...
    this->AxisCircle[i]->GetProperty()->SetOpacity(1.0);
    // this->AxisCircle[i]->GetProperty()->SetEdgeVisibility(true);
    this->AxisCircle[i]->GetProperty()->SetColor(AxisColor[i][0], AxisColor[i][1], AxisColor[i][2]);
    this->AxisCircle[i]->GetProperty()->SetLineWidth(this->AxisLineWidth);
    this->AxisCircle[i]->GetProperty()->SetRepresentationToWireframe();
    this->AxisCircle[i]->GetProperty()->SetAmbient(1);
    // this->AxisCircle[i]->GetProperty()->SetAmbientColor(1, 1, 1);
    if(i == 0){
    ...
    this->AxisCircle[i]->SetBackfaceProperty(this->AxisCircle[i]->GetProperty());
  }

  for(int i = 0; i < 3; ++i){
    // Line Axis
    this->AxisLine[i] = vtkActor::New();
    ...
    this->AxisLineMapper[i] = vtkPolyDataMapper::New();
    this->AxisLineMapper[i]->SetInputConnection(this->AxisLineSource[i]->GetOutputPort());
    this->AxisLine[i]->SetMapper(this->AxisLineMapper[i]);
    this->AxisLine[i]->GetProperty()->SetColor(this->AxisColor[i][0], this->AxisColor[i][1], this->AxisColor[i][2]);
    this->AxisLine[i]->GetProperty()->SetLineWidth(this->AxisLineWidth);
  }
  /*--------------------------------------------------*/
...
int vtkBoxRepresentation::RenderOpaqueGeometry(vtkViewport *v)
{
  ...
  // this->_XCircleActor->SetPropertyKeys(this->GetPropertyKeys());
  ...
  for(int i = 0; i < 3; ++i){
    this->AxisCircle[i]->SetPropertyKeys(this->GetPropertyKeys());
    this->AxisCircle[i]->RenderOpaqueGeometry(v);
    this->AxisLine[i]->SetPropertyKeys(this->GetPropertyKeys());
    this->AxisLine[i]->RenderOpaqueGeometry(v);
  }

  // count += this->_XCircleActor->RenderOpaqueGeometry(v);
  for (int j=0; j<7; j++)
  {
...
int vtkBoxRepresentation::RenderTranslucentPolygonalGeometry(vtkViewport *v)
{
  ...
  for(int i = 0; i < 3; ++i){
    this->AxisCircle[i]->SetPropertyKeys(this->GetPropertyKeys());
    this->AxisCircle[i]->RenderTranslucentPolygonalGeometry(v);
    this->AxisLine[i]->SetPropertyKeys(this->GetPropertyKeys());
    this->AxisLine[i]->RenderTranslucentPolygonalGeometry(v);
  }
...
vtkTypeBool vtkBoxRepresentation::HasTranslucentPolygonalGeometry()
{
  ...
  // result |= this->_XCircleActor->HasTranslucentPolygonalGeometry();
...
void vtkBoxRepresentation::PositionHandles()
{
  ...
  // this->_XCircle->SetCenter(this->Points->GetPoint(8));
  for (int i = 0; i < 7; ++i)
  {
...
void vtkBoxRepresentation::HandlesOn()
{
  ...
  else
  {
    ...
    // this->_XCircleActor->VisibilityOn();
  }
}
...
void vtkBoxRepresentation::SizeHandles()
{
  ...
  // this->_XCircle->SetRadius(radius);
}
...
void vtkBoxRepresentation::RegisterPickers()
{
  ...
  // pm->AddPicker(this->_XCirclePicker, this);
}
